...
result1 = await Contacts.findOne({ "roomid": roomid1 });

console.log("result after finding new userid",result1);

if (!result1){

  result2 = await Contacts.findOne({ "roomid": roomid2 });
  ...
  if(result){
    ...
    return res.send({message: 'Contact created successfully',}).status(200);
  }
...
{
  return res.send({message: 'This contact has been already created',}).status(500);
...
result = await Contacts.findOne({ "roomid": roomid });
console.log("results",result);
if(!result){
  return res  .send({ message: 'Error! no room exists by this room id' }).status(500);
